...
class Node ():
  ...
  It is implemented for the Mover game daemon included with libxgame, but
  other game daemons are supported as well if they:

  * Have the --x_rpc_url, --game_rpc_port and --datadir flags that moverd
    has, and
  * provide at least the "stop" and "getcurrentstate" RPC methods.
  ...
  def __init__ (self, basedir, port, binary):
    ...
    self.log = logging.getLogger ("xgametest.gamenode")
  ...
  def start (self, xrpc, extraArgs=[], wait=True):
