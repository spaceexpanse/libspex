...
XAYAD_BINARY_DEFAULT = "/usr/local/bin/xd"
XCORE_BINARY_DEFAULT = "/usr/local/bin/xx-core"
XETH_BINARY_DEFAULT = "/usr/local/bin/xx-eth"
DEFAULT_DIR = "/tmp"
DIR_PREFIX = "xgametest_"
...
class XGameTest (object):
  ...
  def __init__ (self, gameId, gameBinaryDefault):
    ...
    parser.add_argument ("--xd_binary", default=XAYAD_BINARY_DEFAULT,
                         help="xd binary to use in the test")
    parser.add_argument ("--xcore_binary", default=XCORE_BINARY_DEFAULT,
                         help="xx-core binary to use")
    parser.add_argument ("--xeth_binary", default=XETH_BINARY_DEFAULT,
                         help="xx-eth binary to use")
  ...
  def main (self):
    ...
    logfile = os.path.join (self.basedir, "xgametest.log")
    ...
    self.log = logging.getLogger ("xgametest.testcase")
  ...
  def runXXCoreEnvironment (self):
    ...
    if self.zmqPending != "one socket":
      raise AssertionError ("X-X-Core only supports one-socket pending")

    from xx import core
    env = core.Environment (self.basedir, self.ports,
                            self.args.xd_binary, self.args.xcore_binary)

    with env.run ():
      self.xnode = env.xnode
      self.rpc.spacexpanse = env.xnode.rpc
      yield env

  @contextmanager
  def runXXEthEnvironment (self):
    """
    ...
    if self.zmqPending != "one socket":
      raise AssertionError ("X-X-Eth only supports one-socket pending")
