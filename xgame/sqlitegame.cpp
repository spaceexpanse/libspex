...
private:
   ...
   * from libxgame.  Returns false if not.
   */
...
protected:
  ...
  SetupSchema () override
  {
    ...
    auto& db = GetDatabase ();
    db.Execute (R"(
      CREATE TABLE IF NOT EXISTS `xgame_gamevars`
          (`onlyonerow` INTEGER PRIMARY KEY,
           `gamestate_initialised` INTEGER NOT NULL);
      INSERT OR IGNORE INTO `xgame_gamevars`
          (`onlyonerow`, `gamestate_initialised`) VALUES (1, 0);

      CREATE TABLE IF NOT EXISTS `xgame_autoids` (
          `key` TEXT PRIMARY KEY,
          `nextid` INTEGER NOT NULL
      );
    )");

    /* If the `schema_version` volumn is missing from `xgame_gamevars`,
       ...
       also works with databases created in previous versions of libxgame
       (and just adds it to them as well now).  */
    auto stmt = db.PrepareRo (R"(
      SELECT `name`
        FROM pragma_table_info ('xgame_gamevars')
...
SQLiteGame::Storage::IsGameInitialised (const SQLiteDatabase& db)
{
  ...
  CHECK (stmt.Step ())
      << "Failed to fetch result from xgame_gamevars";
...
SQLiteGame::AutoId::AutoId (SQLiteGame& game, const std::string& key)
{
  auto stmt = game.database->GetDatabase ().Prepare (R"(
    SELECT `nextid` FROM `xgame_autoids` WHERE `key` = ?1
  )");
  stmt.Bind (1, key);
...
SQLiteGame::AutoId::Sync (SQLiteGame& game, const std::string& key)
{
  ...
  auto stmt = game.database->GetDatabase ().Prepare (R"(
    INSERT OR REPLACE INTO `xgame_autoids`
