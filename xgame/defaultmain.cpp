 ...
 */
void
VerifyXVersion (const GameDaemonConfiguration& config, const unsigned v)
{
  LOG (INFO) << "Connected to SpaceXpanse Core version " << v;
  CHECK_GE (v, config.MinXVersion) << "SpaceXpanse Core is too old";
  if (config.MaxXVersion > 0)
    CHECK_LE (v, config.MaxXVersion) << "SpaceXpanse Core is too new";
}
 ...
 */
void
WaitForX (jsonrpc::IClientConnector& conn, const jsonrpc::clientVersion_t v)
{
  LOG (INFO) << "Waiting for SpaceXpanse to be up...";

  XRpcClient client(conn, v);
  while (true)
    try
      {
...
{
  switch (config.XJsonRpcProtocol)
    {
    case 1:
    ...
    default:
      LOG (FATAL) << "Invalid JSON-RPC version: " << config.XJsonRpcProtocol;
    }
}
