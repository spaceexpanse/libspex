...
/* Shared utility functions for unit tests of xgame.  */
...
#include "rpc-stubs/xwalletrpcclient.h"
#include "rpc-stubs/xwalletrpcserverstub.h"
 ...
 */
class MockXRpcServer : public XRpcServerStub
{
...
public:
   ...
   */
  explicit MockXRpcServer (jsonrpc::AbstractServerConnector& conn);

  MockXRpcServer () = delete;
  MockXRpcServer (const MockXRpcServer&) = delete;
  void operator= (const MockXRpcServer&) = delete;
 ...
 */
class MockXWalletRpcServer : public XWalletRpcServerStub
{
...
public:
  ...
  /** The type of the corresponding RPC client.  */
  using RpcClient = XWalletRpcClient;
   ...
   * tests can explicitly specify the calls they want.
   */
  explicit MockXWalletRpcServer (jsonrpc::AbstractServerConnector& conn);

  MockXWalletRpcServer () = delete;
  MockXWalletRpcServer (const MockXWalletRpcServer&) = delete;
  void operator= (const MockXWalletRpcServer&) = delete;
