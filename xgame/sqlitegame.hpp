 ...
 * data in the database and libxgame's state, the underlying SQLiteStorage
 * used by SQLiteGame should be used as the main storage in Game (so that it
 * also holds undo data and the current game state).
 *
 * With this GameLogic implementation, the "game state" as seen by libxgame
 ...
 * (but not through the SQLiteStorage, as that is handled by libxgame).
 *
...
protected:
   ...
   *
   * Note that table names starting with "xgame_" are reserved for use by
   * libxgame itself and must not be used by game implementations.
   */
