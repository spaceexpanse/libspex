...
#include "rpc-stubs/xrpcserverstub.h"
 ...
 */
class MockXRpcServerWithState : public MockXRpcServer
{
...
public:
  ...
  using MockXRpcServer::MockXRpcServer;
...
protected:
  ...
  HttpRpcServer<MockXRpcServerWithState> mockXServer;
  ...
  {
    g.ConnectRpcClient (mockXServer.GetClientConnector (),
...
using XVersionTests = GameTests;

TEST_F (XVersionTests, Works)
{
  Json::Value networkInfo(Json::objectValue);
  networkInfo["version"] = 1020300;
  EXPECT_CALL (*mockXServer, getnetworkinfo ())
      .WillOnce (Return (networkInfo));

  Game g(GAME_ID);
  ConnectToMockRpc (g);
  EXPECT_EQ (g.GetXVersion (), 1020300);
}
...
{
  Game g(GAME_ID);
  mockXServer->SetBestBlock (0, BlockHash (0));
  ConnectToMockRpc (g);
...
TEST_F (ChainDetectionTests, Reconnection)
{
  /* For the death test, we need to make sure that we only run the server
     ...
     in one of them.  */
  mockXServer->StopListening ();

  Game g(GAME_ID);
  EXPECT_DEATH (
    {
      mockXServer->StartListening ();
      mockXServer->SetBestBlock (0, BlockHash (0));
      ConnectToMockRpc (g);
      ConnectToMockRpc (g);
    },
...
TEST_F (DetectZmqEndpointTests, BlocksWithoutPending)
{
  ...
  EXPECT_CALL (*mockXServer, getzmqnotifications ())
...
TEST_F (TrackGameTests, CallsMade)
{
  {
    InSequence dummy;
    EXPECT_CALL (*mockXServer, trackedgames ("add", GAME_ID));
    EXPECT_CALL (*mockXServer, trackedgames ("remove", GAME_ID));
  }
...
protected:
  ...
  InitialStateTests()
    : g(GAME_ID)
  {
    EXPECT_CALL (*mockXServer, getblockhash (GAME_GENESIS_HEIGHT))
...
TEST_F (InitialStateTests, AfterGenesis)
{
  ...
  EXPECT_CALL (*mockXServer, game_sendupdates (GAME_GENESIS_HASH, GAME_ID))
      .WillOnce (Return (upd));

  mockXServer->SetBestBlock (20, BlockHash (20));
  ReinitialiseState (g);
...
TEST_F (InitialStateTests, WaitingForGenesis)
{
  ...
  mockXServer->SetBestBlock (9, BlockHash (9));
  SetStartingBlock (8, BlockHash (8));
  AttachBlock (g, BlockHash (9), emptyMoves);
  EXPECT_EQ (GetState (g), State::PREGENESIS);

  mockXServer->SetBestBlock (10, TestGame::GenesisBlockHash ());
...
TEST_F (InitialStateTests, MissedNotification)
{
  ...
  mockXServer->SetBestBlock (20, TestGame::GenesisBlockHash ());
...
TEST_F (InitialStateTests, UnspecifiedGenesisHash)
{
  ...
  mockXServer->SetBestBlock (10, BlockHash (10));
...
{
  mockXServer->SetBestBlock (GAME_GENESIS_HEIGHT,
...
TEST_F (GetCurrentJsonStateTests, HeightResolvedViaRpc)
{
  ...
  EXPECT_CALL (*mockXServer, getblockheader (GAME_GENESIS_HASH))
...
protected:
  ...
  {
    EXPECT_CALL (*mockXServer, trackedgames (_, _)).Times (AnyNumber ());
    EXPECT_CALL (*mockXServer, getrawmempool ())
...
TEST_F (SyncingTests, CatchingUpForward)
{
  ...
  mockXServer->SetBestBlock (12, BlockHash (12));
  ReinitialiseState (g);
  EXPECT_EQ (GetState (g), State::CATCHING_UP);
  ExpectGameState (TestGame::GenesisBlockHash (), "");
...
TEST_F (SyncingTests, CatchingUpMultistep)
{
  ...
  mockXServer->SetBestBlock (13, BlockHash (13));
  ReinitialiseState (g);
  EXPECT_EQ (GetState (g), State::CATCHING_UP);
  ExpectGameState (TestGame::GenesisBlockHash (), "");
...
{
  EXPECT_CALL (*mockXServer, game_sendupdates (_, GAME_ID))
      .WillOnce (Return (SendupdatesResponse (BlockHash (12), "reqtoken")));
...
TEST_F (GameStorageRetryTests, AttachBlock)
{
  ...
  mockXServer->SetBestBlock (11, BlockHash (11));
